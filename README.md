...
***Note: the following project can be used as a reference for most PIC® devices with some modification.*** 
...
Once inside of the MCC Melody plugin, add an instance of the UART peripheral from the Device Resources: 
...
Also note that the integrated LED is connected to pin RC7 and switch is connected to pin RA0 

Inside of the **Easy View** select UART2 from the UART PLIB Selector.
...
**Note:** The `LED_Toggle()` macro can be viewed in the `pins.h` header file located in `basicUARTcomms_Q71>Header Files>MCC Generated Files>system><code>pins.h`:

![Using the LED_Toggle() Macro](images/togglingTheLED.png)

Next, configure the SW0 interrupt to transmit the character 'S' over the UART when pressed. Navigate to the `pins.c` file in `basicUARTcomms_Q71>Source Files>MCC Generated Files>sytem>src><code>pins.c`. Scroll through the file to locate the `SW0_DefaultInterruptHandler()` ISR named after the custom name added to pin RA0 in the previous section:
...
In order for any interrupt to be used on the PIC18F56Q71, global interrupts must be enabled. This is easily done by navigating to the `basicUARTcomms_Q71>Source Files><code>main.c` and scrolling to the `main()` function. At the top of the function, uncomment the  `//INTERRUPT_GlobalInterruptEnable();` macro by deleting the two forward slashes:
...
The PIC18F56Q71 Curiosity Nano board should be recognized by the Data Visualizer including the associated COM port. Note that your COM port number may differ than what is shown below. 
...
Verify that the UART2 receive interrupt is working by clicking inside of the terminal window and entering the character 'T'. This action should toggle the LED on the Curiosity Nano board ON/OFF. Entering any other character should have no effect.

Verify the SW0 interrupt on change by pressing the switch on the Curiosity Nano board. The character 'S' should be output to the terminal.
